...
class BinarySearchTree:
    ...
    def add(self, key: Any, value: Any) -> bool:
        """키가 key이고 값이 value인 노드를 삽임"""

        def add_node(node: Node, key: Any, value: Any) -> None:
            """node를 루트로 하는 서브트리에 키가 key 이고 값이 value인 노드를 삽입"""
            if key == node.key:
                return False # key가 이진 검색 트리에 이미 존재
            elif key < node.key: # 추가하고자 하는 key가 노드의 key보다 작다면
                if node.left is None: # 왼쪽이 비었다면
                    node.left = Node(key, value, None, None) # 왼쪽 자식 노드로 추가
                else:
                    add_node(node.left, key, value) # 왼쪽 자식을 루트로 서브트리에서 다시 검색
            else: # 추가하고자 하는 key가 노드의 key보다 크다면
                if node.right is None: # 오른쪽이 비었다면
                    node.right = Node(key, value, None, None) # 오른쪽 자식 노드로 추가 
                else:
                    add_node(node.right, key, value) # 오른쪽 자식을 루트로 서브트리에서 다시 검색
            return True

        if self.root is None: # 루트가 비었으면 삽입
            self.root = Node(key, value, None, None) # 새로운 노드 생성
            return True
        else:
            return add_node(self.root, key, value) # add를 위한 검색 시작
